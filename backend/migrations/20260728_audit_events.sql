...
-- public/axerly_app roles are revoked and RLS is enabled with no policies as
-- defense in depth. Without this, a hosted Supabase project's default ACLs
-- leave the whole table readable and writable with the public public key — any
...
-- grant the axerly_app the privileges the backend needs. The explicit grant
-- keeps a fresh plain-Postgres apply working even where axerly_app has no
-- default ACL for new tables.
revoke all on public.audit_events from public;
grant select, insert, update, delete on public.audit_events to axerly_app;
