...
--
-- RLS ENABLE with no policy is not by itself a grant boundary: `public` and
-- `axerly_app` inherit table privileges from PUBLIC unless revoked.
...
--     public.users is the authoritative address; user_profiles can be missing.
...
left join public.users creator_auth on creator_auth.id = p.user_id
...
left join public.users creator_auth on creator_auth.id = r.user_id
...
grant select, insert, update, delete
  on public.tabular_review_legacy_shares to axerly_app;
...
begin
  if exists (
    ...
    execute $sql$
      ...
      left join public.users creator_auth
