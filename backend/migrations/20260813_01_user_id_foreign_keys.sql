...
--
-- Enforce issue #104's missing public.users referential integrity across every
...
alter table public.document_versions
  ...
  foreign key (deleted_by) references public.users(id) on delete set null not valid;
alter table public.workflows
...
alter table public.workflow_shares
  ...
  foreign key (shared_by_user_id) references public.users(id) on delete cascade not valid;
alter table public.default_workflow_installations
...
alter table public.workflow_open_source_submissions
  ...
  foreign key (submitted_by_user_id) references public.users(id) on delete cascade not valid;
alter table public.chats
