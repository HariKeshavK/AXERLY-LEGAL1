...
-- AXERLY direct-Postgres baseline schema.
  ...
  where profile.user_id = p_user_id
    and exists (
      ...
      where auth_user.id = p_user_id
        and auth_user.password_hash is not null
        and auth_user.password_hash::text <> ''
    )
...
drop trigger if exists on_auth_user_email_updated on public.users;
...
-- partitioned HttpOnly session in the embedded Word task pane. Session tokens
...
-- row is already gone). security definer so the public.users probe works
...
begin
  ...
  -- Member's auth user being deleted (cascade from public.users): stand aside.
...
-- ---------------------------------------------------------------------------
--
-- The frontend uses the backend directly for authentication. Application
-- data access goes through the backend API after the backend validates its
-- opaque server-side session. The browser never receives database credentials.

-- Audit history of user actions (queried via the backend only).
-- Defined here — above the axerly_app grant block — so `grant ... on all
-- tables in schema public` below covers it on a fresh install. Like every other
-- backend-owned table, reachable only through the non-superuser app role.
...
create table if not exists public.memory_conversation_activity (
  ...
  latest_turn_actor_user_id uuid references public.users(id) on delete set null,
  project_id uuid,
  project_curator_actor_user_id uuid references public.users(id) on delete set null,
...
alter table public.memory_conversation_activity
  ...
  add column if not exists latest_turn_actor_user_id uuid references public.users(id) on delete set null,
  add column if not exists project_id uuid,
  add column if not exists project_curator_actor_user_id uuid references public.users(id) on delete set null,
...
select 'user', id, false from public.users
...
drop trigger if exists on_auth_user_created_memory on public.users;
...
begin
  if p_surface = 'chat' then
    ...
    perform actor.id from public.users actor
...
-- Access-scope guards are database integrity constraints against mixed direct
-- and organization access. Request authorization remains in lib/authz.ts.
...
revoke all on public.org_members from public;
revoke all on public.org_invitations from public;
revoke all on public.project_access_grants from public;
revoke all on public.project_org_access_overrides from public;
revoke all on public.projects from public;
revoke all on public.project_subfolders from public;
revoke all on public.library_folders from public;
revoke all on public.documents from public;
revoke all on public.document_versions from public;
revoke all on public.upload_sessions from public;
revoke all on public.upload_session_files from public;
revoke all on public.upload_processing_jobs from public;
revoke all on public.document_edits from public;
revoke all on public.workflows from public;
revoke all on public.hidden_workflows from public;
revoke all on public.workflow_shares from public;
revoke all on public.workflow_org_access_overrides from public;
revoke all on public.workflow_open_source_submissions from public;
revoke all on public.mike_workflows from public;
revoke all on public.mike_workflow_assets from public;
revoke all on public.workflow_addons from public;
revoke all on public.chats from public;
revoke all on public.chat_access_grants from public;
...
revoke all on public.word_document_edits from public;
revoke all on public.tabular_reviews from public;
revoke all on public.tabular_review_access_grants from public;
revoke all on public.tabular_review_legacy_shares from public;
revoke all on public.tabular_cells from public;
revoke all on public.tabular_review_rows from public;
revoke all on public.tabular_review_row_sources from public;
revoke all on public.tabular_review_chats from public;
revoke all on public.tabular_review_chat_messages from public;
revoke all on public.user_api_keys from public;
revoke all on public.auth_handoff_tickets from public;
revoke all on public.user_router_models from public;
revoke all on public.user_mcp_connectors from public;
revoke all on public.user_mcp_oauth_tokens from public;
revoke all on public.user_mcp_oauth_states from public;
revoke all on public.user_mcp_connector_tools from public;
revoke all on public.user_mcp_tool_audit_logs from public;
revoke all on public.courtlistener_citation_index from public;
revoke all on public.courtlistener_opinion_cluster_index from public;
revoke all on public.audit_events from public;
revoke all on public.db_jobs from public;
revoke all on public.memory_files from public;
revoke all on public.memory_consolidation_states from public;
revoke all on public.memory_conversation_activity from public;
revoke all on public.memory_conversation_turn_leases from public;
revoke all on public.memory_consolidation_results from public;
...
-- backend connects as axerly_app, so grant it only the data privileges that
-- no browser process receives. RLS is intentionally not used: upstream had no
-- effective policy set and its service role bypassed it. Central authz plus
-- these integrity triggers is the enforceable boundary for this migration.
--
...
-- install would create the table with no axerly_app privileges and the
...
$$;
revoke all on function public.queue_document_version_cleanup() from public;
grant execute on function public.queue_document_version_cleanup() to axerly_app;
...
$$;
revoke all on function public.create_document_version(uuid, jsonb, boolean) from public;
grant execute on function public.create_document_version(uuid, jsonb, boolean) to axerly_app;
...
$$;
revoke all on function public.delete_document_version(uuid, uuid, uuid) from public;
grant execute on function public.delete_document_version(uuid, uuid, uuid) to axerly_app;
...
$$;
revoke all on function public.create_document_versions(jsonb) from public;
grant execute on function public.create_document_versions(jsonb) to axerly_app;
...
$$;
revoke all on function public.activate_document_version(uuid, uuid) from public;
grant execute on function public.activate_document_version(uuid, uuid) to axerly_app;
...
revoke all on function public.document_lifecycle_version()
  from public;
grant execute on function public.document_lifecycle_version() to axerly_app;
