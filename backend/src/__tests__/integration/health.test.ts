...
// Keep route tests isolated from PostgreSQL. Authentication resolves no user,
// simulating an invalid or expired opaque session token.
vi.mock("../../lib/database", () => {
    ...
    return {
        ...
        createRequestDatabase: vi.fn(() => database),
    };
});
...
describe("requireAuth middleware", () => {
    ...
    it("rejects requests with an invalid Bearer token (401)", async () => {
        // The mocked local auth API resolves no user for any token.
        const res = await request(app)
            .get("/chat")
