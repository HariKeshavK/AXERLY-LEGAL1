...
describe("backend architecture", () => {
  ...
  it("authorization consumers import only the central authz boundary", () => {
    const implementationFiles = new Set([
      "lib/access.ts",
      "lib/contentAccess.ts",
      "lib/projectAccess.ts",
      "lib/orgAccessOverrides.ts",
      "lib/authz.ts",
    ]);
    const policyModules = new Set([
      ...
      "lib/contentAccess",
      "lib/projectAccess",
      "lib/orgAccessOverrides",
    ]);
    const offenders = PRODUCTION_FILES
      .filter((file) => !implementationFiles.has(file))
      .flatMap((file) =>
        importsOf(file)
          .filter((specifier) => policyModules.has(specifier))
