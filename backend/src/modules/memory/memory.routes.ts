...
function projectContext(required: Capability): ContextResolver {
  return async (req, res) => {
    const result = await resolveProjectMemoryContext(createDatabase(), {
...
async function sendMemoryError(
  ...
  if (error instanceof MemoryRevisionConflictError && ctx) {
    let current;
    try {
      current = await currentMemory(createDatabase(), ctx);
    } catch {
      current = undefined;
    }
    res.status(409).json({
...
function installMemoryRoutes(
  ...
  router.get("/", asyncRoute(async (req, res) => {
    try {
      ...
      if (!ctx) return;
      res.json(await currentMemory(createDatabase(), ctx));
    } catch (error) {
  ...
  router.put("/", asyncRoute(async (req, res) => {
    ...
    try {
      ...
      if (!ctx) return;
      const current = await saveMemory(createDatabase(), {
        ctx,
  ...
  router.patch("/settings", asyncRoute(async (req, res) => {
    try {
      ...
      if (!ctx) return;
      const current = await setMemoryEnabled(createDatabase(), {
        ctx,
  ...
  if (wipeContext) {
    router.delete("/", asyncRoute(async (req, res) => {
      try {
        ...
        if (!ctx) return;
        res.json(
          await wipeMemory(createDatabase(), {
            ctx,
            updatedBy: res.locals.userId as string,
          }),
        );
