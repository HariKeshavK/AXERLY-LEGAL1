...
import { removeGrantsForEmail } from "../../lib/authz";
import { removeContentGrantsForEmail } from "../../lib/authz";
...
export type AccountDeletionOrgBlocker = {
     ...
     */
    reason: "members" | "content" | "sole_admin";
};
...
export async function listOrgsBlockingAccountDeletion(
    ...
    for (const m of (memberships ?? []) as {
        ...
        const hasContent = await orgOwnsContent(db, m.org_id);
        ...
        blockers.push({
            ...
            name:
                typeof (org as { name?: unknown } | null)?.name === "string"
                    ...
                    : "your organization",
            reason: hasOtherMembers ? "members" : hasContent ? "content" : "sole_admin",
        });
    }
    return blockers;
}
 ...
 *  - The sole admin cannot delete their account even when the firm is empty:
 *    the firm row and its join settings belong to the installation.
