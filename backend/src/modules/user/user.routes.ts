...
// Record password capability only after verifying the local users row.
...
userRouter.delete(
    ...
    asyncRoute(async (_req, res) => {
        const userId = res.locals.userId as string;
        const result = await deletePrivateMemories(createDatabase(), userId);
        if (!result.ok) return void sendInternalError(res, result.error);
        res.status(204).send();
    }),
);
...
userRouter.get(
    ...
    asyncRoute(async (_req, res) => {
        ...
        void recordAudit(createDatabase(), {
            userId,
