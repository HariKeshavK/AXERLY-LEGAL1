...
// make. It never touches req/res: the cookie-bearing database client is
...
//
// There is no `db: Db` here — auth talks to local authentication, not to the
...
import { z } from "zod";
import type { AuthSession, AuthUser } from "../../lib/localAuth";
...
export function startGoogleOAuth(client: Db, redirectTo: string) {
...
export function exchangeCodeForSession(client: Db, code: string) {
...
export function signOut(client: Db, scope: "global" | "local") {
...
export function updatePassword(client: Db, password: string) {
  return client.auth.updateUser({ password });
}

export function listMfaFactors(client: Db) {
  return client.auth.mfa.listFactors();
}

export function mfaAssuranceLevel(client: Db) {
